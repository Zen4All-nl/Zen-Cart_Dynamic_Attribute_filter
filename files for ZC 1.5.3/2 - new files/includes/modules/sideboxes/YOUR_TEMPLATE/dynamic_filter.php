...
// retrieve filtered and unfiltered product options
  ...
  if (count($priceArray) > 0) {
    ...
    for ($start = $min - 0.5; $start < $max; $start = $end + 0.01) {
      ...
      if ($end < $max) {
        $text = $currency_symbol . round($start * zen_get_tax_rate($product_check->fields['products_tax_class_id']) * $conversion_rate) . ' -- ' . $currency_symbol . round($end * zen_get_tax_rate($product_check->fields['products_tax_class_id']) * $conversion_rate);
      } else {
        $text = $currency_symbol . round($start * zen_get_tax_rate($product_check->fields['products_tax_class_id']) * $conversion_rate) . ' and over';
      }
