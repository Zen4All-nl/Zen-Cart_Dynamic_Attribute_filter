...
// Price Range/Attribute filter
  $manufacturerGroup = str_replace(' ', '_', DYNAMIC_FILTER_MANUFACTURER_GROUP);
  $categoryGroup = str_replace(' ', '_', DYNAMIC_FILTER_CATEGORY_GROUP);
  $priceGroup = str_replace(' ', '_', DYNAMIC_FILTER_PRICE_GROUP);
  $prvKey = '';
  $prvHaving = '';
  $filter = '';
  $having = '';
    ...
    if(substr($key,0,strlen(DYNAMIC_FILTER_PREFIX)) == DYNAMIC_FILTER_PREFIX && array_filter($value)) {
      ...
      foreach ($value as $value) {
		...
		// attributes
          ...
          $having .= ' FIND_IN_SET("' . $key . $value .'", GROUP_CONCAT(CONCAT(po.products_options_name, pov.products_options_values_name)))';
		  $filter_attr = true;
		  $prvHaving = $key;
        }
      }
