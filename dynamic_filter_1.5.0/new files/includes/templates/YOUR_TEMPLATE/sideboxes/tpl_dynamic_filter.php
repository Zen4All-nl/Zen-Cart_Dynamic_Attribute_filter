...
// retrieve filtered and unfiltered product options
	...
	if (count($unfilteredManufacturers) > 1) $unfilteredManufacturers = array_unique($unfilteredManufacturers);
	if (count($unfilteredCategories) > 1) $unfilteredCategories = array_unique($unfilteredCategories);
	if (count($unfilteredProducts) > 1) $unfilteredProducts = array_unique($unfilteredProducts);
	if (count($filteredManufacturers) > 1) $filteredManufacturers = array_unique($filteredManufacturers);
	if (count($filteredCategories) > 1) $filteredCategories = array_unique($filteredCategories);
	if (count($filteredProducts) > 1) $filteredProducts = array_unique($filteredProducts);
	if (count($priceArray) > 1) $priceArray = array_unique($priceArray);
	...
	if (PRODUCT_LIST_FILTER == 0) {
		if (isset($_GET['manufacturers_id']) && $_GET['manufacturers_id'] != '' || $current_page_base == 'products_all' || $current_page_base == 'products_new' || $current_page_base == 'specials' || $current_page_base == 'featured_products' || $current_page_base == 'advanced_search_result') {
			if (count($unfilteredCategories) > 0) {
				$group = DYNAMIC_FILTER_PREFIX . str_replace(' ', '_', DYNAMIC_FILTER_CATEGORY_GROUP);
				$resetParms[] = $group;
				...
				if (FILTER_OPTIONS_STYLE == 'Expand' && count($unfilteredCategories) > FILTER_MAX_OPTIONS) $content .= '<a class="dFilterToggle" href="#">More' . zen_image(DIR_WS_TEMPLATE_IMAGES . 'arrow_more.gif', 'More', '', '', 'class="dFilterToggleImg"') . '</a></div></div>';
			}
		} 
		if (!isset($_GET['manufacturers_id'])) {
			if (count($unfilteredManufacturers) > 0) {
				$group = DYNAMIC_FILTER_PREFIX . str_replace(' ', '_', DYNAMIC_FILTER_MANUFACTURER_GROUP);
				$resetParms[] = $group;
				...
				if (FILTER_OPTIONS_STYLE == 'Expand' && count($unfilteredManufacturers) > FILTER_MAX_OPTIONS) $content .= '<a class="dFilterToggle" href="#">More' . zen_image(DIR_WS_TEMPLATE_IMAGES . 'arrow_more.gif', 'More', '', '', 'class="dFilterToggleImg"') . '</a></div></div>';
			}
		}
	}
...
/********************************************start price range link/check boxes***************************************************/
	if (count($priceArray) > 0) {
		...
		$group = DYNAMIC_FILTER_PREFIX . str_replace(' ', '_', DYNAMIC_FILTER_PRICE_GROUP);
		$resetParms[] = $group;
		$parameters = zen_get_all_get_params();
		...
		if (FILTER_OPTIONS_STYLE == 'Expand' && $priceCount > FILTER_MAX_OPTIONS) $content .= '<a class="dFilterToggle" href="#">More' . zen_image(DIR_WS_TEMPLATE_IMAGES . 'arrow_more.gif', 'More', '', '', 'class="dFilterToggleImg"') . '</a></div></div>';
	}
		...
		// output if option name changes!!!
			if ($attributes->fields['products_options_name'] != $savName) {
				...
				if ($savName != "") {
					...
					if (FILTER_OPTIONS_STYLE == 'Expand' && $attrCount > FILTER_MAX_OPTIONS) $content .= '<a class="dFilterToggle" href="#">More' . zen_image(DIR_WS_TEMPLATE_IMAGES . 'arrow_more.gif', 'More', '', '', 'class="dFilterToggleImg"') . '</a></div></div>';
				}

				$group = DYNAMIC_FILTER_PREFIX . str_replace(' ', '_', $attributes->fields['products_options_name']);
				$resetParms[] = $group;
				$parameters = zen_get_all_get_params();
